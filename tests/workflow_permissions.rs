...
//!
//! Every workflow must carry an explicit `permissions:` block at the workflow
//! level, and no permissions grant anywhere in any workflow may exceed
//! `contents: read`. The repository-default token scope is a settings toggle,
//! not a reviewed commit: without an explicit block, flipping that toggle
//! silently widens what a job's GITHUB_TOKEN can do.
//!
//! Measured on main run 30701775073 (2026-08-01), before this guard existed:
//! the `ci` job (Test, Lint, Format), which had no block, ran with
//! `Contents: read, Metadata: read, Packages: read`, while `audit` (Security
//! audit), which carried a job-level block, ran with exactly
//! `Contents: read, Metadata: read`. `publish.yml` — the workflow holding the
//! crates.io publish path — had no block at any level.
//!
...
//! workflow file enters this contract the moment it is committed. Each file is
//! run through a real YAML parser (serde_norway), so a workflow that no longer
//! parses fails here before it fails on a runner.
//!
//! If a workflow ever legitimately needs a wider grant (for example
//! `id-token: write` for trusted publishing), widen the assertion here in the
//! same commit that widens the workflow, so the exception is reviewed and
//! named rather than inherited. Precedent for this shape: slokit
//! `tests/workflow_permissions.rs` and svccat
//! `tests/workflow_permissions_tests.rs`.
...
/// Every permissions mapping in `value` (workflow-level and per-job), returned
/// as (location, mapping-or-None-if-missing-at-workflow-level).
...
/// Guard 1: every workflow declares an explicit workflow-level `permissions:`
/// block, so no job's token scope is inherited from the repository settings
/// toggle.
#[test]
